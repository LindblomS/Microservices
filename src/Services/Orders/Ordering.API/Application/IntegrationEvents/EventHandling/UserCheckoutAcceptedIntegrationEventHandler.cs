            ...
            using (LogContext.PushProperty("IntegrationEvent", $"{@event.Id}-{Program.AppName}"))
            {
                ...
                if (@event.RequestId != Guid.Empty)
                {
                    using (LogContext.PushProperty("IdentifiedCommandId", @event.RequestId))
                    {
                        ...
                        _logger.LogInformation($"----- Sending command: {requestCreateOrder.GetGenericTypeName()} - {nameof(requestCreateOrder.Id)}: {requestCreateOrder.Id} ({requestCreateOrder})");

                        result = await _mediator.Send(requestCreateOrder);

                        if (result)
                            _logger.LogInformation($"----- CreateOrderCommand suceeded - RequestId: {@event.RequestId}");
                        else
                            _logger.LogWarning($"CreateOrderCommand failed - RequestId: {@event.RequestId}");

                    }
                }
                else
                    _logger.LogWarning($"Invalid IntegrationEvent - RequestId is missing - {@event}");
